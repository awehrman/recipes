...
const getNextGroup = (currentGroup: string) => {
  ...
  const next =
    groupIndex !== GROUP_BY.length - 1 ? GROUP_BY[groupIndex + 1] : GROUP_BY[0];
  return next;
};
...
const Filters: React.FC = () => {
  ...
  const linksProps = (condition: boolean) => ({
    ...
    onKeyPress: (e: KeyboardEvent<HTMLElement>) => e.preventDefault(),
    role: 'button',
    tabIndex: 0
  });
  ...
  return (
    <Wrapper>
      ...
      <View>
        <Link
          href={{ pathname: '/ingredients', query: { view: 'all', group } }}
          {...linksProps(view === 'all')}
        >
        ...
        <Link
          href={{ pathname: '/ingredients', query: { view: 'new', group } }}
          {...linksProps(view === 'new')}
        >
      ...
      <GroupBy>
        ...
        <Link
          href={{
            ...
            query: { view, group: getNextGroup(group) }
          }}
          {...linksProps(false)}
        >
